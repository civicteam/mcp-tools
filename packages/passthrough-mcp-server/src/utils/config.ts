 ...
 */
function loadConfigFromFile(filePath: string): MCPHooksConfig | null {
  try {
 ...
 */
function convertMCPHooksConfig(mcpConfig: MCPHooksConfig): Config {
  ...
  const hooks: HookDefinition[] = mcpConfig.hooks.map((hook) => {
    ...
    // Hook names without URLs are not supported by passthrough-mcp-server directly
    // They should be handled by a higher-level bundle that provides the hook instances
    throw new Error(
      `Hook configuration must include a URL. Hook name without URL: ${hook.name || "unnamed"}`,
    );
  });
