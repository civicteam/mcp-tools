...
const USE_STDIO = process.argv.includes("--stdio") || process.env.USE_STDIO === "true";
...
const isSuccess = (result: FetchUrlResult): result is { content: string } => 'content' in result
...
async function fetchUrl(url: string): Promise<FetchUrlResult> {
  try {
    ...
    return { 
      content: contentType.includes('text/html') ? turndownService.turndown(text) : text 
    };
  } catch (error) {
