...
export class GuardrailHook implements Hook {
  ...
  private allowedDomains: string[] = ["example.com", "github.com", "raw.githubusercontent.com"];
  ...
  async processRequest(toolCall: ToolCall): Promise<HookResponse> {
    ...
    if (name.toLowerCase().includes("fetch") || name.toLowerCase().includes("http") || name.toLowerCase().includes("request")) {
      ...
      if (toolArgs.url && typeof toolArgs.url === "string") {
        try {
          ...
          if (!this.allowedDomains.some(domain => url.hostname.endsWith(domain))) {
            return {
              response: "abort",
  ...
  async processResponse(response: any, originalToolCall: ToolCall): Promise<HookResponse> {
    ...
    if (responseStr.length > 1048576) { // 1MB
      return {
        response: "continue",
    ...
    if (typeof response === 'object' && response && 'content' in response) {
      ...
      for (const item of content) {
        if (typeof item === 'object' && item && (item.type === 'image' || item.type === 'file')) {
          console.log(`Validated ${item.type} content in response from '${name}'`);
